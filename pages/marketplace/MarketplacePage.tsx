...
export const MarketplacePage: React.FC = () => {
  ...
  const CountdownTimer = ({ createdAt }: { createdAt: string }) => {
      ...
      return (
          <span className={`font-mono font-bold ${expired || timeLeft.startsWith('0') ? 'text-red-600' : 'text-green-600'}`}>
              {timeLeft}
          </span>
      );
  };
      ...
      <div className="bg-slate-900 pt-8 pb-24 px-4 sm:px-6 lg:px-8 rounded-b-[2.5rem] shadow-xl relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-7xl mx-auto">
              ...
              {activeMainTab === 'browse' && (
                  <div className="bg-white rounded-xl shadow-2xl p-3 border border-slate-200 transform translate-y-8 animate-in fade-in slide-in-from-bottom-4">
                      <form onSubmit={handleFilter} className="flex flex-col lg:flex-row gap-3">
                          ...
                          <div className="lg:w-auto">
                              <button type="submit" disabled={loading} className="w-full h-full min-h-[50px] bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg px-8 flex items-center justify-center gap-2 shadow-lg shadow-blue-600/30 transition-all">
                                  {loading ? <i className="fas fa-spinner fa-spin"></i> : <Search size={20} />} Cari
                              </button>
                          </div>
                      </form>
                  </div>
                          ...
                          <div className="flex-1 p-6 overflow-y-auto">
                              ...
                              <div className="space-y-4">
                                  <div>
                                      ...
                                      <div className="text-3xl font-black text-slate-900">Rp {calculateTotalPrice().toLocaleString('id-ID')}</div>
                                  </div>
